...
const socialLinks = [
  {
    ...
    icon: (
      <svg viewBox="0 0 320 512" xmlns="http://www.w3.org/2000/svg" className="h-6 w-6">
...
export function SocialSidebar() {
  return (
    <div className="fixed top-1/2 -translate-y-1/2 right-0 z-50">
      <div className="max-w-fit rounded-l-2xl flex flex-col items-center justify-center bg-card/10 backdrop-blur-md p-4 shadow-lg transition-all duration-500 hover:bg-card/20">
        <ul className="flex flex-col list-none gap-8 items-center justify-center">
          {socialLinks.map((link) => (
            <li key={link.name} className="relative group cursor-pointer">
              <span className="absolute top-0 left-0 h-14 w-14 rounded-full bg-accent opacity-0 group-hover:opacity-20 transition-all duration-300"></span>
